...
train = df[(df["year"] >= 2000) & (df["year"] < 2013)]
val = df[(df["year"] >= 2013) & (df["year"] < 2017)]

train = train[["x_coord", "y_coord", "water_percentage", "date_time"]]
val = val[["x_coord", "y_coord", "water_percentage", "date_time"]]
...
for i in range(len(grid_parameter)):
    ...
    forecast_values = generate_forecast(model, val_grid_data)
...
performance_df = val.merge(
    df_forecasts, 
    on=["x_coord", "y_coord", "date_time"]
)

performance_df.to_csv("../performance/sarima_validation_predictions.csv")

print("SARIMA model performance on validation set is saved ✅")